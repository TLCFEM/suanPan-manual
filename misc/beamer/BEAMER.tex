...
\begin{figure}[H]
\centering\includegraphics[width=.99\linewidth]{FEM}
...
void Domain::assemble_trial_stiffness() const {
    std::for_each(color_map.begin(), color_map.end(), [&](const vector<unsigned>& color) {
        tbb::parallel_for_each(color.begin(), color.end(), [&](const unsigned tag) {
            factory->assemble_stiffness(get_element(tag)->get_trial_stiffness(), get_element(tag)->get_dof_encoding());
        });
    });
}
\end{cppcode}
...
\centering\includegraphics[width=.99\textwidth]{DKTS3}
\end{minipage}
\end{frame}
\begin{frame}{Benchmark}
